...
//
// Parse the given string and return a parse tree.
//
RefPtr<XMLElement> XMLExpatParser::parse(const std::string& sbuf) {
  //std::cerr << "XMLExpatParser::parse\n";

  if(XML_Parse(parser_, sbuf.c_str(), sbuf.length(), 0) == 0) {
    // we only get here if we had a parse error
    std::cerr << "Parse error on sbuf" << std::endl;
      
    throw MyException
      (std::string("XMLExpatParser::parse(string):  \"") +
       XML_ErrorString(XML_GetErrorCode(parser_)) + "\"");
  }
